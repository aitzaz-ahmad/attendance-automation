...
| 4. Event publication | `attendance_etl.messaging`, orchestrated by `attendance_etl.pi4.workflow` | Review-period requests, review-sheet requests, and attendance record payloads | Workflow messages handed to the messaging boundary | Implemented with transitional attendance record payloads |
...
- Current status: the canonical `AttendanceEvent` model is produced by transformation. The event schema and
  current backend-compatible Pub/Sub payload shape are owned by
  [Canonical Attendance Event](contracts/canonical-attendance-event.md).
- Input: normalised source/device records.
- Output: filtered canonical attendance events.
- Limitation: runtime publishing still sends the current backend-compatible payload defined by the canonical
  attendance event contract, not a richer future canonical wire payload.

### 4. Event Publication

The ingestion client hands workflow messages to the messaging boundary for backend processing.

- Purpose: hand off review-period, review-sheet, and attendance-record work to serverless handlers.
- Current implementation: the Pi workflow publishes backend-compatible dictionaries through the current
  messaging implementation. The target boundary is the project-owned `Messenger` abstraction described in
  [Messaging Model](specifications/messaging-model.md).
- Input: workflow request data from the ingestion state machine.
- Output: workflow messages consumed by Google Cloud Functions.
- Limitation: message payloads are transitional dictionaries. Payload schema ownership belongs to
  [Canonical Attendance Event](contracts/canonical-attendance-event.md) and related contracts, not this
...
Current runtime validation is limited to operational checks and expected field access. Examples include
timestamp filtering in the ingestion client, date parsing for review periods, duplicate avoidance based on the
latest stored timestamp, and worksheet existence handling in Google Sheets workflows.

The canonical attendance event is the intended internal payload contract. It is documented separately in
[Canonical Attendance Event](contracts/canonical-attendance-event.md). Messaging transport contracts,
including topic vocabulary and routing, are documented in
[Messaging Model](specifications/messaging-model.md). Future adoption should make canonicalisation explicit
before event publication and add schema enforcement at the appropriate owning boundary.

## Current Limitations

- Runtime attendance payloads still use the backend-compatible payload shape defined by the canonical
  attendance event contract rather than a richer future canonical wire payload.
