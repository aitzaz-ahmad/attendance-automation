...
CREDS = ServiceAccountCredentials.from_json_keyfile_name(GDRIVE_CREDS_JSON, SCOPES)
...
COMPARISON_CONDITION = "LTE({}, {})".format(CHECK_IN_AVERAGE_CELL, CUT_OFF_TIME_CELL)
...
def pub_last_stored_timestamp(device_id, timestamp):
    ...
    future_response = publisher.publish(topic_path, payload.encode("utf-8"), location=device_id)
...
def generate_query_params(value_ip_option, insert_data_option=None):
    query_params = {}
...
def compute_data_range(worksheet_name, elements_per_row, first_row=1, last_row=1):
    last_column = "A" * (elements_per_row // 26)
    ascii_val = (elements_per_row % 26) + 64
    last_column += chr(ascii_val)
    return "{}!A{}:{}{}".format(worksheet_name, first_row, last_column, last_row)


def store_attendance_records(review_sheet_id, review_start_date, worksheet_suffix, attendance_records):
    """
    ...
    if len(stored_records) == 0:
        ...
        recent_time = datetime.strptime(stored_records[-1]["Timestamp"], "%d-%m-%Y %H:%M:%S")

    last_stored_timestamp = recent_time.strftime("%d-%m-%Y %H:%M:%S")
    records_to_store = []
    for record in attendance_records:
        entry_timestamp = datetime.strptime(record["timestamp"], "%d-%m-%Y %H:%M:%S")
    ...
    # for Google Sheets
    data_range = compute_data_range(worksheet_name, RAW_DATA_SHEET_COLUMNS)
    query_params = generate_query_params("USER_ENTERED", "OVERWRITE")
    request_body = generate_request_body(data_range, "ROWS", records_to_store)
    spreadsheet.values_append(data_range, query_params, request_body)

    return last_stored_timestamp


def filter_and_store_attendance(review_sheet_id, review_start_date, device_location, raw_attendance_records):
    """
    ...
    staff_records = [record for record in raw_attendance_records if " " in record["username"]]
    store_attendance_records(review_sheet_id, review_start_date, RAW_DATA_SHEET_STAFF, staff_records)
    print("storing attendance records of employees...")
    employee_records = [record for record in raw_attendance_records if " " not in record["username"]]
    last_stored_timestamp = store_attendance_records(
        review_sheet_id, review_start_date, device_location, employee_records
    )
    # format the raw data for computing and storing the daily attendance
    print("last stored attendance record's timestamp: " + last_stored_timestamp)
...
def format_attendance_records(review_sheet_id, start_date, device_location):
    ...
    """
    raw_attendance_data = load_raw_data(review_sheet_id, start_date, device_location)
...
def load_existing_employees(review_sheet_id):
    ...
    try:
        ...
        print(WORKSHEET_NOT_FOUND_EXCEPTION.format(WEEKLY_SUMMARY, spreadsheet.title))
...
def load_cut_offs(unknown_employees):
    ...
    try:
        ...
        print(WORKSHEET_NOT_FOUND_EXCEPTION.format(WORKSHEET_CUT_OFF_TIMES, spreadsheet.title))
    except Exception as e:
        print("Runtime exception encountered: {}".format(e))
    finally:
...
def identify_and_add_new_hires(review_sheet_id, device_location, formatted_attendance):
    """
    ...
    unknown_employees = mark_unknown_employees(old_employees, formatted_attendance)
                ...
                if employee in formatted_attendance:
                    ...
                    if calendar_date in employee_attendance:
                        ...
                        if entry_type in daily_attendance:
                            ...
                            ret_val = update_entry_timestamp(stored_timestamp, timestamp, entry_type)
                            if ret_val is True:
...
def update_daily_attendance(review_sheet_id, formatted_attendance_data):
    ...
    spreadsheet = client.open_by_key(review_sheet_id)
    worksheet = spreadsheet.worksheet(DAILY_ATTENDANCE)
    compiled_attendance = compile_daily_attendance(worksheet, formatted_attendance_data)
    ...
    data_range = compute_data_range(worksheet.title, len(compiled_attendance[0]), last_row=len(compiled_attendance))
    query_params = generate_query_params("USER_ENTERED")
    request_body = generate_request_body(data_range, "ROWS", compiled_attendance)
    spreadsheet.values_update(data_range, query_params, request_body)


def calculate_weekly_stats(cumulative_checkins, days_worked, total_work_duration):
    """
    ...
    weekly_check_in_avg = TIME_DURATION_FORMAT.format(hours, minutes, seconds)
    weekly_hours_worked = TIME_DURATION_FORMAT.format(worked_hours, worked_minutes, worked_seconds)
    ...
    for row in range(2, len(daily_attendance_data), 3):
        ...
        for column in range(2, len(daily_attendance_data[0])):
            ...
            if timestamp != "":  # ensure that the employee checked in on this date
                ...
                hours_worked = datetime.strptime(daily_hours_worked, "%H:%M:%S")
                ...
                sum_hours_worked += timedelta(
                    hours=hours_worked.hour,
                    minutes=hours_worked.minute,
                    seconds=hours_worked.second,
                )

            calendar_date = datetime.strptime(daily_attendance_data[0][column], "%d-%m-%Y").date()
            if calendar_date.isoweekday() == 7:  # denotes Sunday, marks end of the week
                if days_worked > 0:
                    ...
                    avg_check_in, weekly_hours = calculate_weekly_stats(
                        sum_check_in_time, days_worked, sum_hours_worked
                    )
...
def update_weekly_summary(review_sheet_id):
    ...
    weekly_summary_data = compile_weekly_summary(worksheet, compiled_weekly_stats)

    # batch write the compiled weekly summary to avoid exceeding the quota
    # limit of 100 write requests per user per 100 seconds for Google Sheets
    data_range = compute_data_range(worksheet.title, len(weekly_summary_data[0]), last_row=len(weekly_summary_data))
    query_params = generate_query_params("USER_ENTERED")
    request_body = generate_request_body(data_range, "ROWS", weekly_summary_data)
    spreadsheet.values_update(data_range, query_params, request_body)
...
def entry_point(event, context):
    ...
    last_stored_timestamp = filter_and_store_attendance(
        review_sheet_id, review_start_date, device_location, raw_attendance_records
    )
    formatted_attendance = format_attendance_records(review_sheet_id, review_start_date, device_location)
    identify_and_add_new_hires(review_sheet_id, device_location, formatted_attendance)
