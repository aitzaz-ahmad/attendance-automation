...
def create_spreadsheet(review_month, review_year, employees, start_date, end_date):
    ...
    try:
        ...
        raw_data_staff = spreadsheet.add_worksheet(title=RAW_DATA_STAFF, rows=DEFAULT_ROWS, cols=DEFAULT_COLS)
        raw_data_dk = spreadsheet.add_worksheet(title=RAW_DATA_DK, rows=DEFAULT_ROWS, cols=DEFAULT_COLS)
        raw_data_sh = spreadsheet.add_worksheet(title=RAW_DATA_SHADMAN, rows=DEFAULT_ROWS, cols=DEFAULT_COLS)
        spreadsheet.add_worksheet(title=DAILY_ATTENDANCE, rows=len(employees) * 3 + 2, cols=DEFAULT_COLS)
        spreadsheet.add_worksheet(title=WEEKLY_SUMMARY, rows=len(employees) * 3 + 2, cols=MINIMUM_COLS)
        ...
        create_daily_attendance_template(spreadsheet, DAILY_ATTENDANCE, start_date, end_date, employees)
        create_weekly_summary_template(spreadsheet, WEEKLY_SUMMARY, start_date, end_date, employees)

        # share the spreadsheet with the appropriate personnel
        spreadsheet.share(INTERNAL_TOOLS, perm_type=PER_LEVEL_USER, role=ROLE_WRITER)
        spreadsheet.share(SARAH_ANWAR, perm_type=PER_LEVEL_USER, role=ROLE_WRITER)
        spreadsheet.share(RAFIA_MASUD, perm_type=PER_LEVEL_USER, role=ROLE_WRITER)
        spreadsheet.share(IQRA_AMJAD, perm_type=PER_LEVEL_USER, role=ROLE_WRITER)
        spreadsheet.share(MUDDASSIR_MUNEER, perm_type=PER_LEVEL_USER, role=ROLE_WRITER)
        spreadsheet.share(SELF, perm_type=PER_LEVEL_USER, role=ROLE_READER)
...
def create_daily_attendance_template(spreadsheet, worksheet_name, start_date, end_date, employees):
    ...
    request_body = generate_request_body(data_range, "ROWS", [row_title, row_days])
...
def create_weekly_summary_template(spreadsheet, worksheet_name, start_date, end_date, employees):
...
def entry_point(event, context):
    ...
    start_date = datetime.strptime(query_params["start_date"], "%m/%d/%Y")
    review_year = query_params["sheetname"]
    review_month = query_params["month"]
    print("attendance review sheet request for {} {}".format(review_month, review_year))
    ...
    if attendance_review_sheet is None:
        ...
        attendance_review_sheet = create_spreadsheet(review_month, review_year, employees_info, start_date, end_date)
