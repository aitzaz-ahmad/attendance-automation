...
# global variables
HANDLER_TABLE: Dict[int, Callable[[], None]] = {}
PI4_STATE = 0
SYSTEM_FLAGS = 0
DEVICE_INFO = None
...
def get_attendance_records(from_date, to_date=None):
    ...
    users, records = pull_records_from_device(DEVICE_INFO["ip"], DEVICE_INFO["port"])
...
def create_subscription(topic_name, subscription_name):
    ...
    does not exist
    """
    already_exists = subscription_exists(topic_name, subscription_name)
...
def transition_state(new_state):
    ...
    if (
        PI4_STATE != AWAIT_REVIEW_PERIOD
        and PI4_STATE != AWAIT_REVIEW_SHEET
        and PI4_STATE != AWAIT_LAST_STORED_TIMESTAMP
    ):
        capture_snapshot()
...
def handler_fetch_review_period():
    ...
    # period
    publish_message_to_topic(PROJECT_ID, GET_REVIEW_PERIOD_TOPIC, REVIEW_PERIOD_INFO)
...
def handler_await_review_period():
    ...
    """
    global REVIEW_PERIOD_INFO, SYSTEM_FLAGS
    subscription_name = SUBSCRIPTION_NAME.format(NEW_REVIEW_PERIOD_TOPIC, DEVICE_INFO["identifier"])
...
def handler_request_review_sheet():
    ...
    publish_message_to_topic(PROJECT_ID, CREATE_REVIEW_SHEET_TOPIC, REVIEW_PERIOD_INFO)
...
def handler_await_review_sheet():
    ...
    """
    global REVIEW_SHEET_ID
    subscription_name = SUBSCRIPTION_NAME.format(NEW_REVIEW_SHEET_TOPIC, DEVICE_INFO["identifier"])
...
def handler_relay_attendance_records():
    ...
    if LAST_STORED_TIMESTAMP is not None:
        from_timestamp = datetime.strptime(LAST_STORED_TIMESTAMP, "%d-%m-%Y %H:%M:%S")
    else:
    ...
    if len(new_records) > 0:
        ...
        publish_message_to_topic(PROJECT_ID, STORE_ATTEND_RECORDS_TOPIC, request_params)
...
def handler_await_last_stored_timestamp():
    ...
    """
    global LAST_STORED_TIMESTAMP
    subscription_name = SUBSCRIPTION_NAME.format(LAST_STORED_TIMESTAMP_TOPIC, DEVICE_INFO["identifier"])
...
def load_snapshot():
    ...
    """
    global PI4_STATE, SYSTEM_FLAGS, REVIEW_SHEET_ID, LAST_STORED_TIMESTAMP
    ...
    print(
        "loaded snapshot: \n",
        msg.format(PI4_STATE, REVIEW_SHEET_ID, LAST_STORED_TIMESTAMP),
    )
